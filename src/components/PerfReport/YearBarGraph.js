    ...
    var monthCols = months.map((d, i) => {
      return (
        <div key={d} className='monthCol'>
          <div className='dataBar' style={{height: (100 - (this.props.data[i] / max)*100)  + '%', backgroundColor:'transparent'}} />
          <div className='dataBar' style={{height: (this.props.data[i] / max) * 100 +'%', backgroundColor: this.props.color}} />
        </div>
      )
    })
    var monthLabel = months.map(d=> {
      return (
        <div key={d} className='monthLabel'>{d[0]}</div>
      )
    })
    return (
